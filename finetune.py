...
epochs = 2 ** 0         # range(2, 4)
batch_size = 2 ** 4     # range(5, 7)
learning_rate = 0.02    # range(0.01, 0.1)
...
tune_model.load(f'weights\weights_cnn\e8bpe781.25traimgs50000valimgs10000_weights.npz')
...
#save model weights
tune_model.save(f'weights\weights_{tune_model.type}_ft\e{epochs}bpe{batches_per_epoch}traimgs{train_imgs}valimgs{val_imgs}_weights.npz')

#graphs
dashboard = DashboardTrainee(tune_model)
dashboard.plot_training_curves()
dashboard.plot_all_overlays()
#dashboard.plot_all_layer_metrics()