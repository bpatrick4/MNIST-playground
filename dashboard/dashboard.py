...
class DashboardTrain:
  ...
  def plot_layer_Metrics_cln(self):
    layer_hist = self.trainer.history['layers']
  ...
  def plot_all_layer_metrics(self):
      layer_hist = self.trainer.history['layers']

      # Collect all metric names that appear anywhere
      all_metrics = set()
      for metrics in layer_hist.values():
          for name, values in metrics.items():
              if len(values) > 0:
                  all_metrics.add(name)

      all_metrics = sorted(all_metrics)  # consistent ordering
      num_metrics = len(all_metrics)
      num_layers = len(layer_hist)

      # Build figure
      fig, axes = plt.subplots(
          num_layers, num_metrics,
          figsize=(5 * num_metrics, 3.5 * num_layers),
          squeeze=False,
          sharex=True
      )

      for row_idx, (layer_name, metrics) in enumerate(layer_hist.items()):
          for col_idx, metric_name in enumerate(all_metrics):

              ax = axes[row_idx][col_idx]

              # If this metric has data for this layer, plot it
              if metric_name in metrics and len(metrics[metric_name]) > 0:
                  ax.plot(metrics[metric_name])
                  ax.set_title(f"{layer_name} – {metric_name.replace('_',' ').title()}")
                  ax.grid(True)
                  if "grad" in metric_name:
                     ax.set_yscale('log')
              else:
                  # Hide empty subplot
